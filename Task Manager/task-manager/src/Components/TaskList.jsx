...
export default function TaskList() {
  const { filteredTasks = [], updateTaskStatus } = useTaskContext();  // Provide a default empty array
        ...
        <div key={task._id} className={`relative rounded-md shadow-md ${getStatusColor(task.status)}`}>
          <div className="p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={() => toggleDropDown(task._id)} className="text-gray-500 hover:text-gray-700">
          ...
          {isDropDownOpen(task._id) && (
            <div className="absolute top-full right-0 mt-2 w-48 bg-white border rounded-md shadow-md z-10">
              <button className="block w-full py-2 px-4 text-left hover:bg-gray-100" onClick={() => handleEdit(task._id, task.title, task.description)}>
                Edit
              </button>
              <button className="block w-full py-2 px-4 text-left text-red-600 hover:bg-red-100" onClick={() => handleDelete(task._id)}>
                Delete
              </button>
              {task.status !== 'Completed' && (
                <button className="block w-full py-2 px-4 text-left hover:bg-gray-100" onClick={() => handleComplete(task._id)}>
                  Mark as Completed
                </button>
              )}
            </div>
